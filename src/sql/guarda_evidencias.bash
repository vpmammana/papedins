...
# log de erros
log_de_erros_guarda="log_de_erros_guarda_guarda_evidencias.txt"
> "$log_de_erros_guarda"
...
if mysql -u root -ptoninho13 -e "use papedins_db; show tables like 'evidencias'" | grep 'evidencias'> /dev/null 2>> "$log_de_erros_guarda"; then
...
if mysql -u root -ptoninho13 -e "use papedins_db; show tables like 'autores_evidencias'" | grep 'autores_evidencias'> /dev/null 2>> "$log_de_erros_guarda"; then
...
if mysql -u root -ptoninho13 -e "use papedins_db; show tables like 'evidencias_tipos_de_identificadores'" | grep 'evidencias_tipos_de_identificadores'> /dev/null 2>> "$log_de_erros_guarda"; then
mysql -u root -ptoninho13 papedins_db -N -e 'select CONCAT("INSERT INTO evidencias_tipos_de_identificadores (nome_evidencia_tipo_de_identificador, id_tipo_de_identificador, id_evidencia, valor, id_chave_externa, time_stamp) VALUES \n(\"", COALESCE(nome_evidencia_tipo_de_identificador,""), "\", (select id_chave_tipo_de_identificador from tipos_de_identificadores where nome_tipo_de_identificador=\"", COALESCE((select nome_tipo_de_identificador from tipos_de_identificadores where id_chave_tipo_de_identificador = id_tipo_de_identificador),""),"\"), (select id_chave_evidencia from evidencias where nome_evidencia = \"", (select nome_evidencia from evidencias where id_chave_evidencia = id_evidencia),"\" and data = \"",(select data from evidencias where id_chave_evidencia=id_evidencia),"\"), \"", COALESCE(valor,""), "\", NULL, \"", COALESCE(time_stamp,""), "\");\n\n") from evidencias_tipos_de_identificadores' -B | sed 's/\\n/\n/g'
else
...
if mysql -u root -ptoninho13 -e "use papedins_db; show tables like 'enderecos_do_OSM'" | grep 'enderecos_do_OSM'> /dev/null 2>> "$log_de_erros_guarda"; then
...
echo -e "\n\nAGORA VAMOS VER SE TEM enderecos_evidencias\n" >> "$log_de_erros_guarda"
mysql -u root -ptoninho13 papedins_db -e "select * from enderecos_evidencias;" >> "$log_de_erros_guarda"

echo -e "\n\n\n# ===========================\n"
echo -e "# insert into enderecos_evidencias\n"
if mysql -u root -ptoninho13 -e "use papedins_db; show tables like 'enderecos_evidencias'" | grep 'enderecos_evidencias'> /dev/null 2>> "$log_de_erros_guarda"; then
mysql -u root -ptoninho13 papedins_db -N -e 'select CONCAT("INSERT INTO enderecos_evidencias (id_evidencia, id_endereco_do_OSM, time_stamp) VALUES \n((select id_chave_evidencia from evidencias where nome_evidencia=\"", (select nome_evidencia from evidencias where id_chave_evidencia = id_evidencia),"\" and data=\"",(select data from evidencias where id_chave_evidencia=id_evidencia),"\"), (select id_chave_endereco_do_OSM from enderecos_do_OSM where osm_type = \"", COALESCE((select osm_type from enderecos_do_OSM where id_chave_endereco_do_OSM = id_endereco_do_OSM), ""),"\" and osm_id = \"",COALESCE((select osm_id from enderecos_do_OSM where id_chave_endereco_do_OSM = id_endereco_do_OSM), ""),"\" and class = \"",COALESCE((select class from enderecos_do_OSM where id_chave_endereco_do_OSM = id_endereco_do_OSM), ""),"\"), \"", COALESCE(time_stamp,""), "\");\n\n") from enderecos_evidencias' -B | sed 's/\\n/\n/g'
else
